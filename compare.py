...
report=docsLib+"/compare_report.csv"
...
def runCompare():
   ...
   if sql == "":
      ...
      outPut("Database A is Empty")
   elif dbb == "":
      outPut("Database B is Empty")
   else:
      rbvSelect = rbv.get()
      if rbvSelect == 1:
         outPut("List Compare")
...
compareButton.place(x=(wdt * .4),y=(hgt * .1))

clearButton=Button(mainCanvas,text="Clear", width=10, fg="red", command=clearText)
clearButton.place(x=(wdt * .6),y=(hgt * .1))

dbaLabel=Label(mainCanvas, text='Database A : ', fg="black", anchor=W, justify=LEFT, bg="light grey", width=30)
...
dbbLabel=Label(mainCanvas, text='Database B : ', fg="black", anchor=W, justify=LEFT, bg="light grey", width=30)
...
sqlAreaLabel=Label(mainCanvas, text='SQL to use for COMPARE : ', fg="black", anchor=W, justify=LEFT, bg="light grey", width=30)
